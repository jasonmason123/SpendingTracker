...
using SpendingTracker_API.DTOs.Statistics;
...
namespace SpendingTracker_API.Controllers
{
    ...
    public class StatisticsController : ControllerBase
    {
        ...
        [HttpGet("get-expense-by-category-custom-range")]
        public async Task<IActionResult> GetExpenseByCategoryCustomRange([FromQuery] DateTime dateFrom, [FromQuery] DateTime dateTo)
        {
            try
            {
                var transactions = await _unitOfWork.Transactions.GetListAsync(selector => new Transaction
                {
                    ...
                    {
                        Name = selector.TransactionCategory.Name
                    }
                }, new TransactionFilterParams
                {
                    DateFrom = dateFrom,
                ...
                var result = transactions
                    .GroupBy(x => x.TransactionCategory.Name)
        ...
        [HttpGet("get-top-recent")]
        public ActionResult<IEnumerable<TransactionDto>> GetTopRecentTransactions([FromQuery] int top = 3)
        {
            try
            {
                var list = _unitOfWork.Transactions.GetPagedList(1, top);
